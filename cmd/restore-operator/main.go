...
import (
	...
	controller "github.com/objectrocket/sensu-operator/pkg/controller/restore-operator"
...
const (
	serviceNameForMyself = "sensu-restore-operator"
	servicePortForMyself = 19999
)

func init() {
	flag.BoolVar(&createCRD, "create-crd", true, "The restore operator will not create the SensuRestore CRD when this flag is set to false.")
	flag.Parse()
}
...
func main() {
	...
	logrus.Infof("sensu-restore-operator Version: %v", version.Version)
...
func run(stop <-chan struct{}) {
	...
	if err != nil {
		logrus.Fatalf("sensu restore operator stopped with error: %v", err)
	}
}
