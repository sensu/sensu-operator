...
func (r *Restore) processItem(key string) error {
	...
	return r.handleCR(obj.(*api.SensuRestore), key)
}

// handleCR takes in SensuRestore CR and prepares the seed so that etcd operator can take over it later.
func (r *Restore) handleCR(er *api.SensuRestore, key string) (err error) {
	...
	// NOTE: Since the restore SensuCluster is created with the same name as the SensuClusterRef,
	// the seed member will send a request of the form /backup/<cluster-name> to the backup server.
	// The SensuRestore CR name must be the same as the SensuCluster name in order for the backup server
	// to successfully lookup the SensuRestore CR associated with this <cluster-name>.
	if er.Name != er.Spec.SensuCluster.Name {
		err = fmt.Errorf("failed to handle restore CR: SensuRestore CR name(%v) must be the same as SensuCluster name(%v)", er.Name, er.Spec.SensuCluster.Name)
		return err
	}
...
func (r *Restore) reportStatus(rerr error, er *api.SensuRestore) {
	if rerr != nil {
...
// - fetches and deletes the reference SensuCluster CR
// - creates new SensuCluster CR with same metadata and spec as the reference CR
...
// 	- ownerRef to above SensuCluster CR
// - update SensuCluster CR spec.paused=false
// 	- sensu operator should pick up the membership and scale the sensu cluster
func (r *Restore) prepareSeed(er *api.SensuRestore) (err error) {
	defer func() {
		if err != nil {
	...
	// Need to delete sensu pods, etc. completely before creating new cluster.
	r.deleteClusterResourcesCompletely(ecRef.Name)

	// Create the restored SensuCluster with the same metadata and spec as reference SensuCluster
	...
	if err != nil {
		return fmt.Errorf("failed to update sensucluster CR to spec.paused=false: %v", err)
	}
	return nil
}

func (r *Restore) createSeedMember(ec *api.SensuCluster, svcAddr, clusterName string, owner metav1.OwnerReference) error {
	m := &etcdutil.Member{
