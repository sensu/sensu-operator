...
func (r *Restore) run(ctx context.Context) {
	...
	r.queue = workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "sensu-restore-operator")
	r.indexer, r.informer = cache.NewIndexerInformer(source, &api.SensuRestore{}, 0, cache.ResourceEventHandlerFuncs{
		AddFunc:    r.onAdd,
