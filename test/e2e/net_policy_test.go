...
func TestClusterNetworkPolicy(t *testing.T) {
	...
	err = e2eutil.WaitUntilPodSuccess(f.KubeClient, client, f.Namespace, 60*time.Second)

	// We schould receive an error here with the networkpilicies deployed
	if err != nil {
		t.Log("success: cannot connect to the testSensu pod from the test-network-client-pod")
	}

	err = e2eutil.DeleteNetworkPolicy(f.KubeClient, "default")
	if err != nil {
		t.Fatal(err)
	}
}
